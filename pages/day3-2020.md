...
:aside: The tuple "pi@\<ip_address\>" means that we are logging in the remote machine with the specified IP address using the user "pi". The username depends on the account you want to use when you login.
...
With `ssh` you can remotely access a computer. However, you may also want to send or retrieve files to and from that computer. To do so you can use the command `scp` as such:

```bash
scp <path_to_local_file> pi@<remote_ip_address>:<path_to_destination>
```

In the example below you will copy the file "log.txt" in the current directory (represented by "./") to the remote machine with the IP address "10.0.22.1", saving it in the location "/home/pi/Documents/"
